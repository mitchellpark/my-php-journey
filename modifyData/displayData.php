    ...
    <body>
        ...
        <div id="content">
            ...
            if($_SERVER['REQUEST_METHOD']=="POST"){
                for($i=0; $i<sizeof($colNames); $i++){
                    if(isChecked($colNames[$i])){
                        $checkedCols[] = $conn->real_escape_string($colNames[$i]);
                    }
                }
                if(isset($_REQUEST["orderBy"])){
                    $orderBy = $conn->real_escape_string($_REQUEST["orderBy"]);
                }
                if(isset($_REQUEST["ascOrDesc"])){
                    if($_REQUEST["ascOrDesc"]=="descending"){
                        $orderMethod = $conn->real_escape_string("desc");
                    }
                }
            }
            ...
            function printTable($arr, $orderBy, $orderMethod){
                ...
                /* checked columns, order method, and column to order    by are initialized in the sql command*/
                $sql = "SELECT id, ";
